...
class CNNClassifier(nn.Module):
    def __init__(self,
        ...
        with gzip.open(pretrained_embeddings_path, "rt") as fh:
            ...
            for line in fh:
                ...
                if word in token_to_index:
                    embeddings_matrix[token_to_index[word]] =                        torch.FloatTensor([float(n) for n in vector.split()])
        ...
        self.convs = []
        for filter_l in filters_length:
            self.convs.append(
                nn.Conv1d(vector_size, filters_count, filter_l)
            )
